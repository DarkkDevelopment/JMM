...
export const attendanceSlice = createSlice({
    ...
    reducers: {
        ...
        setEmployeeHodorTime: (state, action: PayloadAction<{ index: number, time: string }>) => {
            ...
            employee.TotalNumberOfWorkingHoursAtThatDay = calcualateWorkingHours(employee.HodoorTime, employee.EnserafTime);
            employee.ExtraHours = calculateAdditionalHours(state.workingHoursConstant, employee.TotalNumberOfWorkingHoursAtThatDay)
            employee.LateHours = calculateAbsenceHours(state.workingHoursConstant, employee.TotalNumberOfWorkingHoursAtThatDay)
        },
        setEmplyeeEnsrafTime: (state, action: PayloadAction<{ index: number, time: string }>) => {
            let employee = state.employees[action.payload.index]
        ...
        setKhasmHourRatio: (state, action: PayloadAction<{ index: number, value: any }>) => {
            state.employees[action.payload.index].LateFactor = action.payload.value;
        },
        setHafezHourRatio: (state, action: PayloadAction<{ index: number, value: any }>) => {
