...
export const fetchGhyabByDate = createAsyncThunk(
  ...
  async (date: Date) => {
    ...
    const agazarResponse = await axios.post(
      "/api/HR_Endpoints/vacations/getVacationsAtThatDay",
      {
        date,
      }
    );
  ...
  reducers: {
    ...
    removeGhyab: (state, action: PayloadAction<{ id: number }>) => {
      state.employees = state.employees.filter(
        (e) => e.PersonCode !== action.payload.id
      );
    },
  },
...
export const { setGhyab, removeGhyab } = ghyabSlice.actions;
