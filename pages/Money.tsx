...
function Money(props: InferGetServerSidePropsType<typeof getServerSideProps>) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const response = await axios.post("/api/payrol/getPayrolHistory", {
        month: monthNo,
    ...
    const checkForPayrol = async () => {
      ...
      const response = await axios({
        method: "POST",
        url: "/api/payrol/checkIfPayrolExists",
        data: {
                    ...
                    {filteredEmployees
                      ...
                      .map((employee) => (
                        <tr key={employee.PersonCode} className="p-10">
                          ...
                          <td className="justify-center p-4 text-right border-b-2 ">
                            {employee.TotalValueOfDarayebAtThatMonth}
                          </td>
                          <td className="justify-center p-4 text-right border-b-2 ">
...
export async function getServerSideProps(context: any) {
  const getNewInsurance = await fetch(
    `${process.env.NEXT_PUBLIC_HOST}/api/insurance/getAll`,
    {
      headers: {
