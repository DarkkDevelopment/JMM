...
function Vacations(
  ...
  useEffect(() => {
    if (searchterm == "") {
      ...
      let backupEmployee = filteredEmployees.filter((obj: Vacatoion) => {
        if (obj.PersonCode.toString().includes(searchterm)) return obj;
        if (obj.PersonName.includes(searchterm)) return obj;
      });
      setFilteredEmployees(backupEmployee);
    }
  ...
  return (
    <div className="flex flex-row bg-gray-100 ">
      <div className="flex justify-center m-10 pr-10 font-display basis-5/6">
  ...
  await Promise.all(
    employees.map(async (emp: IAllEmployeesModel) => {
      let resp = await fetch(
        `${process.env.NEXT_PUBLIC_HOST}/api/vacations/getVacationsHistory`,
        {
          method: "POST",
          headers: {
      ...
      let agazaLimitResp = await fetch(
        `${process.env.NEXT_PUBLIC_HOST}/api/vacations/getVacationsLimit`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
